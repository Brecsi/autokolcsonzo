...
function getLoans(res, carId) {
    return new Promise((resolve, reject) => {
      let sql = `
      SELECT id, loanId, carId, loanStart, numberOfDays, dailyRate from loans
      WHERE carId = ?`;
  ...
  app.get("/cars/:id", (req, res) => {
    const id = req.params.id;
    let sql = `
      SELECT id, license, type, year, color FROM cars
      WHERE id = ?`;
  ...
  app.post("/cars", (req, res) => {
    ...
    pool.getConnection(function (error, connection) {
      ...
      connection.query(
        sql,
        [newR.name, newR.license, newR.type, newR.year, newR.color],
        function (error, result, fields) {
          sendingPost(res, error, result, newR);
        }
      );
  ...
  app.put("/cars/:id", (req, res) => {
    ...
    pool.getConnection(function (error, connection) {
      ...
      connection.query(
        sql,
        [newR.name, newR.license, newR.type, newR.year, newR.color, id],
        function (error, result, fields) {
          sendingPut(res, error, result, id, newR);
        }
      );
