...
app.get("/users", (req, res) => {
  let sql = `select id, firstName, lastName, gender, userName, email, number from users`;
...
app.get("/carsAvailability", (req, res) => {
  let sql = `select DISTINCT c.id as carId, c.license, c.type, c.year, c.color, c.dailyRate,  'Available' as Available from cars c 
...
app.post("/loans", (req, res) => {
  ...
  let sql = `
    INSERT into loans 
    (loanId, carId, loanStart, numberOfDays)
    VALUES
    ( ?, ?, ?, ?)
    `;
  pool.getConnection(function (error, connection) {
    ...
    connection.query(
      sql,
      [newR.loanId, newR.carId, newR.loanStart, newR.numberOfDays],
      function (error, result, fields) {
        sendingPost(res, error, result, newR);
      }
    );
